...
template<class LVT>
std::vector<result_t> Index<LVT>::query_bmm(std::set<std::string> &query, bool conj, size_t top_k)
{
	struct pending_result_t {
		docid_t docid;
		score_t score;
	...
	upper_bounds.push_back(posting_lists_its.begin()->get_lexicon_value().bm25_sigma);
	for(auto it = posting_lists_its.begin(); it != posting_lists_its.end(); ++it)
	{
		if(it == posting_lists_its.begin())
			continue;

		upper_bounds.push_back(upper_bounds.back() + it->get_lexicon_value().bm25_sigma);
	}
